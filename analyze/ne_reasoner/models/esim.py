...
from analyze.ne_reasoner import blocks

tf.disable_v2_behavior()


class EsimModel(object):
    def __init__(self, seq_length, emb_dim, hidden_dim, embeddings, emb_train, num_labels=3):
        ...
        ## Define parameters
        self.Embeddings = tf.Variable(embeddings, trainable=emb_train)

        self.W_mlp = tf.Variable(tf.random_normal([self.dim * 8, self.dim], stddev=0.1))
        self.b_mlp = tf.Variable(tf.random_normal([self.dim], stddev=0.1))

        self.W_cl = tf.Variable(tf.random_normal([self.dim, num_labels], stddev=0.1))
        self.b_cl = tf.Variable(tf.random_normal([num_labels], stddev=0.1))
