...
def analyze(filename: str, workers: int, count: int, svdk: int, save: bool,
            output: bool) -> None:
    """
    ...
    if output:
        ...
        for i in range(len(documents)):
            for j in range(len(documents)):
                beforecomparisons[i, j] = 1 - spatial.distance.cosine(
                    docs[:, i].todense(), docs[:, j].todense())
    ...
    if output:
        ...
        for i in range(len(documents)):
            for j in range(len(documents)):
                aftercomparisons[i, j] = 1 - spatial.distance.cosine(docs[:, i],
                                                                     docs[:, j])
...
def matrix_comparison(u, s, vt, words, documents, output):
    ...
    if not error:
        ...
        printfunc = lambda i: print(
            '{}) {}\t{}'.format(np.abs(i), r[i], documents[r[i]]))
        if output:
...
def open_documents(filename: str, size: int, workers: int) -> typing.Tuple[
    np.ndarray, int]:
