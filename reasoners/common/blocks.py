...
def biLSTM(inputs, dim, seq_len, name):
    ...
    with tf.name_scope(name):
        ...
        # blah=tf.keras.layers.Bidirectional(tf.keras.layers.RNN(
        #
        # ))
        ...
        # hidden_states, cell_states =   nn.bidirectional_dynamic_contrib.rnn(
        hidden_states, cell_states = nn.bidirectional_dynamic_rnn(
            cell_fw=lstm_fwd,
            cell_bw=lstm_bwd,
            inputs=inputs,
...
def LSTM(inputs, dim, seq_len, name):
    ...
    with tf.name_scope(name):
        ...
        hidden_states, cell_states = tf.nn.dynamic_rnn(cell, inputs=inputs, sequence_length=seq_len, dtype=tf.float32,
                                                       scope=name)
